...
async function insertNewSessionForAccount(accountUri, sessionUri, tokenInfo) {
  // Cleanup previous session data (if any)
...
async function selectCurrentOauthSession(session) {
  const queryResult = await query(`
    PREFIX mu: <http://mu.semte.ch/vocabularies/core/>
    PREFIX session: <http://mu.semte.ch/vocabularies/session/>
    PREFIX oauth: <http://data.rollvolet.be/vocabularies/oauth-2.0/>

    SELECT ?sessionId ?oauthSession ?oauthSessionId
    WHERE {
      GRAPH <${sessionsGraph}> {
          ${sparqlEscapeUri(session)} session:account ?account ;
                   mu:uuid ?sessionId .
          ?oauthSession mu:uuid ?oauthSessionId ;
                               oauth:authenticates ${sparqlEscapeUri(session)} ;
                               oauth:tokenValue ?oauthToken ;
                               oauth:expirationDate ?expirationDate .
      }
    }`);
  ...
  if (queryResult.results.bindings.length) {
    ...
    return {
      sessionUri: session,
      sessionId: result.sessionId.value,
      oauthSessionUri: result.oauthSession.value,
      oauthSessionId: result.oauthSessionId.value,
    };
  } else {
    return { sessionUri: null, sessionId: null, oauthSessionUri: null, oauthSessionId: null };
  }
};

async function updateSessionModificationDate(sessionUri) {
  await update(`
    PREFIX dct: <http://purl.org/dc/terms/>
    DELETE WHERE {
      GRAPH <${sessionsGraph}> {
        ${sparqlEscapeUri(sessionUri)} dct:modified ?modified .
      }
    }
  `);
  ...
  await update(`
    ...
    INSERT DATA {
      GRAPH <${sessionsGraph}> {
        ${sparqlEscapeUri(sessionUri)} dct:modified ${sparqlEscapeDateTime(now)} .
      }
    }
  `);
}
...
export {
  ...
  selectCurrentOauthSession,
  updateTokenInfoForSession,
  removeSession,
  updateSessionModificationDate,
  getAllOAuthSessions
}
